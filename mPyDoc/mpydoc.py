...
This is modified version of `pydoc` used to generate documentation for
projects containing `@Pipe` functions, such as those functions in `mPyPl`
library.
...
"""
__all__ = ['help']
__author__ = "Ka-Ping Yee <ping@lfw.org>, Dmitry Soshnikov <dmitri@soshnikov.com>"
...
<tr><td bgcolor="%s">%s</td><td>%s</td>''' % (bgcol, marginalia, gap)
    ...
    def docmodule(self, object, name=None, mod=None, *ignored):
        ...
        for key, value in inspect.getmembers(object, isdata):
            if visiblename(key, all, object) and isinstance(value,pipe.Pipe):
                pfuncs.append((key, value))
                pdict[key] = '#-'+key
                pdict[value] = pdict[key]

        data = []
        for key, value in inspect.getmembers(object, isdata):
            if visiblename(key, all, object) and not isinstance(value,pipe.Pipe):
        ...
        if pfuncs:
            contents = []
            for key, value in pfuncs:
                contents.append(self.document(value.function, key, name, fdict, cdict))
            result = result + self.bigsection(
                'Pipe functions', '#ffffff', '#eeaa77', ' '.join(contents))
...
""")
        ...
        if pfuncs:
            contents = []
            for key, value in pfuncs:
                contents.append(self.document(value.function, key, name))
            result = result + self.section('PIPE FUNCTIONS', '\n'.join(contents))
